import { Prisma } from "@prisma/client";
import { prisma } from "@/lib/prisma";

export type CategoryAdminRow = {
  id: string;
  name: string;
  imageUrl: string | null;
  createdAt: Date;
  productCount: number;
};

export async function findAllCategoriesForAdmin(): Promise<CategoryAdminRow[]> {
  const rows = await prisma.category.findMany({
    select: {
      id: true,
      name: true,
      imageUrl: true,
      createdAt: true,
      _count: { select: { products: true } },
    },
    orderBy: { name: "asc" },
  });
  return rows.map((row) => ({
    id: row.id,
    name: row.name,
    imageUrl: row.imageUrl,
    createdAt: row.createdAt,
    productCount: row._count.products,
  }));
}

export async function findCategoryWithProductCount(
  categoryId: string,
): Promise<{ id: string; productCount: number } | null> {
  const row = await prisma.category.findUnique({
    where: { id: categoryId },
    select: {
      id: true,
      _count: { select: { products: true } },
    },
  });
  if (!row) {
    return null;
  }
  return { id: row.id, productCount: row._count.products };
}

export async function insertCategory(name: string, imageUrl: string | null): Promise<CategoryAdminRow> {
  const row = await prisma.category.create({
    data: { name, imageUrl },
    select: {
      id: true,
      name: true,
      imageUrl: true,
      createdAt: true,
    },
  });
  return { ...row, productCount: 0 };
}
export async function deleteCategoryById(categoryId: string): Promise<void> {
  await prisma.category.delete({
    where: { id: categoryId },
  });
}

export function isPrismaRecordNotFound(error: unknown): boolean {
  return error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2025";
}
